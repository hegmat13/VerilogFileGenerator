...
void verilogSim::ReadCommandsFromFile() {
	...
	cout << _inputFileName; 
	cout << _outputFileName; 
	cout << inputFileStream.is_open(); 
...
unsigned int verilogSim::TestValid() {
	...
	cout << _inputs.at(0).GetVariableI(); 
	if (_inputs.at(0).GetVariableI() == "-1") {
		return (3); 
	}
    ...
    while (i < lenE) {
        ...
        if ((temp.GetOperation() == "+") || (temp.GetOperation() == "-") || (temp.GetOperation() == "*") || (temp.GetOperation() == ">") || (temp.GetOperation() == "") || (temp.GetOperation() == "<") || (temp.GetOperation() == "?") || (temp.GetOperation() == ">>") || (temp.GetOperation() == "<<") || (temp.GetOperation() == "==") || (temp.GetOperation() == "%")) {
...
void verilogSim::TimeSlotScheduling() {
	...
	for (j = 0; j < lenE; j++) {
		...
		for (i = 0; i < lenI; i++) {
			if ((_inputs.at(i).GetVariableI() == _equations.at(j).GetOut()) || (_inputs.at(i).GetVariableI() == _equations.at(j).GetOut())) {
				...
				else if ((_equations.at(j).GetOperation() == "/") || (_equations.at(j).GetOperation() == "%")) {
					_inputs.at(i).SetTimeSlotASAPI(_equations.at(j).GetTimeSlotASAPE() + 2);
				}
				else {  
	...
	for (j = lenE - 1; j >= 0; j--) {
		...
		if (j < lenE - 1) {
			for (k = 0; k < lenE; k++) {
				check = 0;
				for (l = 1; l < (lenE - j - 1); l++) {
					if ((_equations.at(k).GetOut() == _equations.at(j + l).GetFirst()) || (_equations.at(k).GetOut() == _equations.at(j + l).GetSecond())) {
						if (_equations.at(j + l).GetTimeSlotALAPE() < equationTimeSlotALAP) {
							equationTimeSlotALAP = _equations.at(j + l).GetTimeSlotALAPE();
							check = 1;
						}
		...
		if ((_equations.at(j).GetOperation() == "/") || (_equations.at(j).GetOperation() == "%")) {
			equationTimeSlotALAP = equationTimeSlotALAP - 3;
		}
		...
		for (i = 0; i < lenW; i++) {
			...
			if (_equations.at(j).GetOperation() == "?") {
				if ((_wires.at(i).GetVariableW() == _equations.at(j).GetFirst()) || (_wires.at(i).GetVariableW() == _equations.at(j).GetSecond()) || (_wires.at(i).GetVariableW() == _equations.at(j).GetMuxSel())) {
...
void verilogSim::ListRTime() {

	int lenE = _equations.size();
	int i = 0;
	int state = 1;
	int slack = 0, firstOccALU = 0, firstOccMult = 0, prevMult = 0;
	int numALU = 0, numMult = 0, numALUTotal = 1, numMultTotal = 1;
	for (state = 1; state <= _latency; state++) {
		if (numALU > numALUTotal) {
			numALUTotal = numALU;
		}
		if (numMult > numMultTotal) {
			numMultTotal = numMult;
		}
		firstOccALU = -1;
		firstOccMult = -1;
		numALU = 0;
		numMult = 0;
		for (i = 0; i < _equations.size(); i++) {
			if (_equations.at(i).GetTimeSlotASAPE() == state) {
				if (firstOccALU == -1) {
					firstOccALU = i;
				}
				if (firstOccMult == -1) {
				...
				slack = _equations.at(i).GetTimeSlotALAPE() - _equations.at(i).GetTimeSlotASAPE();
				if (slack == 0) {
					_equations.at(i).SetTimeSlotListRE(state);
					if (_equations.at(i).GetOperation() == "*") {
						numMult++;
					}
					else {
						numALU++;
					...
					_equations.at(i).SetTimeSlotASAPE(_equations.at(i).GetTimeSlotASAPE() + 1);
				}
			}
		}
		int k = 1, ready = 0;
		while (numALU < numALUTotal) {
			for (i = 0; i < lenE; i++) {
				if ((_equations.at(i).GetOperation() != "*") && ((_equations.at(i).GetTimeSlotASAPE() - k) == state)) {
					_equations.at(i).SetTimeSlotListRE(state);
					_equations.at(i).SetTimeSlotASAPE(state);
					_equations.at(i).SetTimeSlotALAPE(state);
					numALU = numALUTotal;
					ready = 1;
					break;
				}
		...
		k = 1;
		ready = 0;
		while ((numMult < numMultTotal) && (prevMult < numMultTotal)) {
			for (i = 0; i < lenE; i++) {
				if ((_equations.at(i).GetOperation() == "*") && ((_equations.at(i).GetTimeSlotASAPE() - k) == state)) {
					_equations.at(i).SetTimeSlotListRE(state);
					_equations.at(i).SetTimeSlotASAPE(state);
					_equations.at(i).SetTimeSlotALAPE(state);
					numMult = numMultTotal;
					ready = 1;
					break;
				}
			}
		...
		//if (numALU < numALUTotal) {
		//	for (i = 0; i < lenE; i++) {
		//		if ((_equations.at(i).GetOperation() != "*") && (_equations.at(i).GetTimeSlotASAPE() == state)) {
		//			_equations.at(i).SetTimeSlotListRE(state);
		//			_equations.at(i).SetTimeSlotALAPE(state);
		//			break;
		//		}
...
void verilogSim::WriteCommandsToFile() {
	cout << "FUCKKKKKKKKK" << _equations.at(0).GetTimeSlotListRE() << endl; 
	cout << _equations.at(1).GetTimeSlotListRE() << endl;
	cout << _equations.at(2).GetTimeSlotListRE() << endl;
	...
	for (auto w = 0; w < _wires.size(); w++) {
		...
		//cout << "LOOOOOOK HEREEEE: " << _wires.size() << endl; 
		//cout << "WIREEEE VARIABLE NAMEEE: " << _wires.at(w).GetVariableW() << endl;
	...
	outputFile << "reg [5:0] state;" << endl;
	outputFile << "parameter start_wait = 0;" << endl;


	for (int tosi = 1; tosi < _latency; tosi++) {
		outputFile << "parameter state_" << tosi << " = " << tosi << ";" << endl;
	}

	outputFile << "parameter Final = " << _latency << ";" << endl; 


	outputFile << "always@ (posedge Clk) begin" << endl;
	outputFile << "if ( Rst ==1 ) begin" << endl;

	// conditional that iterates through all variabels except inputs and sets them to zero
	//	template; outfile << '"' << _variable << " <=" << 0 << ';"'
	// end of conditional	
	for (int i = 0; i < _wires.size(); i++) {
		outputFile << _wires.at(i).GetVariableW() << " <=" << "0" << ";" << endl;
	}

	for (int i = 0; i < _outputs.size(); i++) {
		outputFile << _outputs.at(i).GetVariableO() << " <=" << "0" << ";" << endl;
	}

	outputFile << "Done <= 0;" << endl;
	outputFile << "state <= start_wait;" << endl;
	outputFile << "end" << endl;
	outputFile << "else begin" << endl;
	outputFile << "case( state )" << endl;
	outputFile << "start_wait : begin" << endl;
	outputFile << "if(Start == 1) begin" << endl;
	outputFile << "state <= state_1;" << endl;
	outputFile << "Done <= 0;" << endl;
	outputFile << "end" << endl;
	outputFile << "else begin" << endl;
	outputFile << "state <= start_wait;" << endl;
	...
	for (int state = 1; state < _latency; state++) {
		outputFile << "state_" << state << " : begin" << endl;
	for (auto f = 0; f < _equations.size(); f++) {
		if (state == _equations.at(f).GetTimeSlotListRE()) {
...
//		}
//		else if ((_equations.at(f).GetOperation() == "*") && (sign == 0)) {
//			outputFile << "MUL #(" << (bitWidth) << ") Mult" << f << "(" << first << ", " << second << ", " << out << ");" << endl;
//		}
//		else if ((_equations.at(f).GetOperation() == "*") && (sign == 1)) {
//			outputFile << "SMUL #(" << (bitWidth) << ") SMult" << f << "(" << first << ", " << second << ", " << out << ");" << endl;
//		}
//		else if ((_equations.at(f).GetOperation() == ">") && (sign == 0)) {
...
if (_equations.at(f).GetOperation() != "?") {
	outputFile << _equations.at(f).GetOut() << " <= " << _equations.at(f).GetFirst() << " " << _equations.at(f).GetOperation() << " " << _equations.at(f).GetSecond() << ";" << endl;
}
else {
	outputFile << _equations.at(f).GetOut() << " <= " << _equations.at(f).GetMuxSel() << " ? " << _equations.at(f).GetFirst() << " : " << _equations.at(f).GetSecond() << ";" << endl;
}
		...
		if (state == _latency - 1) {
			outputFile << "state <= Final;" << endl;
		}
		else {
			outputFile << "state <= state_" << state + 1 << ";" << endl;
		}
		outputFile << "end" << endl;
}
outputFile << "Final: begin" << endl << "Done <= 1; " << endl << "state <= start_wait;" << endl << "end" << endl;
outputFile << "endcase" << endl << "end" << endl << "end" << endl;
