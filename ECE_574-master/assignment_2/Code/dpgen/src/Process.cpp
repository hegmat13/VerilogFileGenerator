...
int Process::parseNetlist(ifstream &netList, ofstream &verilogFile)
{
	...
	cout << "Critical Path: " << CalculateCiritcalPath(circuitCompVector) << "ns" <<endl;
...
int Process::writeInstantiations(vector<CircuitComp> &components, ofstream &verilogFile)
{
    ...
    for (vector<int>::size_type i = 0; i != components.size(); i++)
    {
        ...
        if((components.at(i).type == "MUX") || (components.at(i).type == "SMUX"))
		{
			lineStream << components.at(i).type << "2x1 #(.DATAWIDTH(" << components.at(i).width << ")) inst" << i;
		}
		else
		{
